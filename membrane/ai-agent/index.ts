 ...
 * 
 * Note: this program is not actively maintained, but we are excited about AI agents in Membrane!
 * Reach out on Discord (https://discord.gg/4RHyJDV8kj) or by email (contact@membrane.io) if you're interested.
 */
...
async function performAction(ref) {
  return await nodes.mem.action({ gref: ref });
}
...
async function performQuery(ref, queryFields) {
  return await nodes.mem.query({ ref, query: `{${queryFields}}` });
}

async function loadUserPrograms() {
  try {
    const programs = await nodes.mem.programs
