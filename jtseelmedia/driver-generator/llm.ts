...
`;
    ...
    for (let attempt = 0; attempt < this.maxRetries; attempt++) {
      try {
        ...
        if (attempt === this.maxRetries - 1) {
          console.error(`All ${this.maxRetries} attempts failed. Throwing error.`);
          throw error;
        }
  ...
  async complete(prompt: string, cacheableContent: { text: string; type: "system" | "user" }[] = []): Promise<string> {
    ...
    return this.retry(async () => {
      const systemMessages = cacheableContent
        .filter(content => content.type === "system")
        .map(content => ({
          type: "text" as const,
          text: sanitizeInput(content.text),
      ...
      const response = await nodes.anthropic.complete({
        ...
        system: [
          {
            type: "text",
            text: this.generalSystemMessage,
          //  cache_control: { type: "ephemeral" }
          },
          ...systemMessages
        ],
