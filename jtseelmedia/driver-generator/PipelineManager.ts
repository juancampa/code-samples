  ...
  async validateDriver(name: string): Promise<ValidationResult> {
    ...
    try {
      ...
      if (validation.isValid) {
        ...
        // Group errors by component for clearer output
        const errorsByComponent = validation.errors.reduce((acc, error) => {
          acc[error.component] = acc[error.component] || [];
      ...
      return {
        isValid: false,
        errors: [{ component: 'code', message: error.message, severity: 'error' }]
      };
    }
  }
    ...
    try {
      ...
      while (iterations < MAX_ITERATIONS) {
        ...
        validation.errors.forEach(error => {
          ...
          console.log(`${severity} [${error.component.toUpperCase()}] ${error.message}`);
          if (error.suggestion) {
            console.log(`   💡 Suggestion: ${error.suggestion}`);
          }
        });
  
        if (!validation.improvementPlan || !validation.improvementPlan.suggestions) {
          console.warn('⚠️ No improvement suggestions provided by validator');
          break;
        }
  
        console.log('\n🔧 Improvement Plan:', {
          components: validation.improvementPlan.components,
          numSuggestions: validation.improvementPlan.suggestions.length
        });
  
        console.log(`\n🔄 Applying improvements for iteration ${iterations + 1}`);
        
        try {
          // Extract component-specific suggestions from the improvement plan
          for (const component of validation.improvementPlan.components) {
            console.log(`📝 Improving ${component}...`);
            
            // Find the suggestion string for this component
            const componentSuggestion = validation.improvementPlan.suggestions.find(s => 
              s.startsWith(`${component.toUpperCase()} Improvements:`)
            );
  
            if (!componentSuggestion) {
              console.warn(`No suggestions found for ${component}, skipping`);
              continue;
            }
  
            const improved = await this.improveComponent(
              driver,
              component,
              componentSuggestion
            );
  
            driver.files = { ...driver.files, ...improved };
  
            // Log the improvement result
            const fileKey = fileForComponent[component];
            console.log(`Improvement result for ${component}:`, {
              file: fileKey,
              beforeLength: driver.files[fileKey]?.length || 0,
              afterLength: improved[fileKey]?.length || 0,
              changed: improved[fileKey] !== driver.files[fileKey]
            });
          }
          ...
          this.checkpointManager.createCheckpoint(
            driver,
            `Improvements applied iteration ${iterations + 1}`,
            validation
          );
